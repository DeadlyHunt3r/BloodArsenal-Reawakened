...
import static deadlyhunter.bloodarsenalreawakened.common.item.ModItems.*;
...
public class GeneratorItem extends ItemModelProvider
{
    ...
    {
        super(generator, BloodArsenalReawakened.MOD_ID, existingFileHelper);
    }
    ...
    {
        return "Blood Arsenal Reawakened item models";
    }

    @Override
    protected void registerModels()
    {
        Set<Item> items = Registry.ITEM.stream().filter(i -> BloodArsenalReawakened.MOD_ID.equals(Registry.ITEM.getKey(i).getNamespace()))
    ...
    private void registerItemBlocks(Set<BlockItem> itemBlocks)
    {
        takeAll(itemBlocks, i -> i.getBlock() instanceof PaneBlock).forEach(i ->
        {
            ...
            withExistingParent(name, "item/generated")
                    .texture("layer0", BloodArsenalReawakened.rl("block/" + baseName));
        });
        ...
        withExistingParent(bloodInfusedWoodFenceName, "block/fence_inventory")
                .texture("texture", BloodArsenalReawakened.rl("block/blood_infused_planks"));
        itemBlocks.remove(ModBlocks.BLOOD_INFUSED_WOOD_FENCE.get().asItem());

        itemBlocks.forEach(i -> {
            String name = Registry.ITEM.getKey(i).getPath();
            withExistingParent(name, BloodArsenalReawakened.rl("block/" + name));
        });
    }
    ...
    {
        return withExistingParent(name, HANDHELD).texture("layer0", BloodArsenalReawakened.rl("item/" + name));
    }
    ...
    private ItemModelBuilder generatedItem(String name)
    {
        return withExistingParent(name, GENERATED)
                .texture("layer0", BloodArsenalReawakened.rl("item/" + name));
    }
    ...
    public static <T> Collection<T> takeAll(Set<? extends T> src, T... items)
    {
        ...
        for (T item : items)
        {
            ...
            {
                BloodArsenalReawakened.LOGGER.warn("Item {} not found in set", item);
            }
        }

        if (!src.removeAll(ret))
        {
            BloodArsenalReawakened.LOGGER.warn("takeAll array didn't yield anything ({})", Arrays.toString(items));
        }
    ...
    public static <T> Collection<T> takeAll(Set<T> src, Predicate<T> pred)
    {
        ...
        if (ret.isEmpty())
        {
            BloodArsenalReawakened.LOGGER.warn("takeAll predicate yielded nothing", new Throwable());
        }
